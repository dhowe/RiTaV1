...
/**
 * Implementation of the Porter Stemming Algorithm by Martin Porter
 * 
 * Transforms a word into its root form.  The input
...
public class PorterStemmer implements StemmerIF
{
   ...
   * the word stemmed is expected to be in lower case.
   */
  ...
  {
    System.out.println("PorterStemmer.main()");
    PorterStemmer ps = new PorterStemmer();
    String[] s = {"run", "runs", "ran", "running", "stemming", "bummer" };
    for (int i = 0; i < s.length; i++)
      System.out.println(ps.stem(s[i]));
    String test = "Stemming is funnier than a bummer";
    System.out.println(ps.stem(test));
    s = RiTa.tokenize(test);
    for (int i = 0; i < s.length; i++)
      System.out.println(ps.stem(s[i]));   
  }

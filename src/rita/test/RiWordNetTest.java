...
public class RiWordNetTest
{
	...
	{  
		...
		String[] exp = { "angle","browse","divine","drag","dredge","feel","finger","fish","fumble","gather","go after","grope","grub","hunt","leave no stone unturned","look for","pursue","quest after","quest for","scour","seek","seek out","shop","surf","want", };
	...
	public void testGetExamplesInt()  
	{
		...
		String[] expected3 ={ "spent many happy days on the beach", "a happy smile", "a happy marriage" };
		...
		expected3 =new String[]{ "spent many happy days on the beach", "a happy smile", "a happy marriage" };
...
//		printArr(w.getAllMeronyms("body", "n"));
	...
	public void testGetHolonymsStringString()
	{
		w.ignoreUpperCaseWords(false);
		w.ignoreCompoundWords(false);
		String[] expected = { "human","body","organic structure","homo","human being","man", };
...
//		printArr(w.getCoordinates("tree", "v"));
	...
	public void testGetAllCoordinates()
	{
		...
		String[] expected = { "remainder", "constant", "base", "biquadrate", "pagination", "linage", "divisor", "natural number", "oxidation number", "quotient", "square", "prime", "no.", "quartic", "subtrahend", "fourth power", "floating-point number", "cardinal", "score", "baryon number", "integer", "minuend", "biquadratic", "decimal", "ordinal", "radix", "dividend", "root", "co-ordinate", "ordinal number", "Fibonacci number", "third power", "multiplier", "multiplicand", "factor", "complex quantity", "imaginary number", "prime quantity", "compound number", "paging", "oxidation state", "quota", "complex number", "atomic number", "lineage", "whole number", "cube", "record", "page number", "second power", "cardinal number", "count", "folio", "cardinality", "arity", "augend", "composite number", "imaginary", "addend", "difference", "multiplier factor", "fixed-point number" };
		setEqualMulti(expected,"getAllCoordinates","coordinate","n");

		String[] expected2 = { "sit","dwell","stay away","keep one's eyes off","attend","pass","come","meet","reach","suffer","poke out","rest","stand back","inhabit","find","cover","straddle","see","belong","run","keep one's hands off","touch","stretch","fill","reach out","continue","occupy","lie","center on","extend","know","go","lead","sit around","go to","keep one's distance","feel","extend to","follow","enjoy","endure","experience","face","populate","witness","stretch along", };
...
//		printArr(w.getAllCoordinates("tree", "v"));
	...
	public void testGetVerbGroupStringString()
	{
		...
		String[] expected2 = { "give-up the ghost", "drop dead", "choke", "give out", "pop off", "break", "pass", "pass away", "go bad", "expire", "buy the farm", "exit", "break down", "go", "cash in one's chips", "conk out", "give way", "fail", "kick the bucket", "croak", "decease", "snuff it", "conk", "perish" };
		//printArr(w.getVerbGroup("die", "v"));
		setEqualMulti(expected2,"getVerbGroup","die","v");

		String[] expected3 = { "go", "last", "hold out", "be", "live", "subsist", "live on", "hold up", "exist", "endure" };
...
//		printArr(w.getVerbGroup(i4[0]));
	...
	{
		String[] expected = { "be","go","hold out","inhabit","know","exist","subsist","last","survive","hold up","experience","endure","dwell","live on","populate", };
		//w.ignoreCompoundWords(false);println(w.getAllVerbGroups("live", "v"),true);
...
////println(w.getSenseIds("air-cooled", "a"));
	...
	public void testGetSimilarInt()
	{
		...
		String[] expected3 = { "precooled", "air-cooled", "water-cooled", "air-conditioned", "caller" };
    ...
    //printArr(w.getSimilar(72540775));
		...
		expected = new String[]{"blessed", "blissful", "bright", "golden", "halcyon", "prosperous", "laughing", "riant"};
		result = w.getSimilar(71151786);
		//println(w.getSenseIds("happy", "a"));
		//println(result);
		setEqual(expected, result);

		expected3 =  new String[]{ "precooled", "air-cooled", "water-cooled", "air-conditioned", "caller" };
	...
	public void testGetAllSimilar()
	{
		...
		String[] expected3 = { "composed", "fashionable", "caller", "precooled", "air-cooled", "stylish", "satisfactory", "cold", "air-conditioned", "water-cooled", "unresponsive", "unqualified" };
	...
	public void testIsCompound()  // DCH: ok now...
	{
		...
		String[] input3 = {"back", "space", "back space", "back_space","","#$%^&","air-cooled"};
  ...
  /*
  public void testGetBaseFormStringString()
  {
    String stem = w.getBaseForm("cakes","n");
    equal(stem, "cake");

    String[] tests = { "run", "runs", "running" };
    for (int i = 0; i < tests.length; i++)
      equal(w.getBaseForm(tests[i],"v"), "run");

    stem = w.getBaseForm("gases","n");
    equal(stem, "gas");    
    equal(w.getBaseForm("buses","n"), "bus");
    equal(w.getBaseForm("happiness","n"), "happiness");
    equal(w.getBaseForm("terrible","a"), "terrible");
    
    equal(w.getBaseForm("terribl","a"), null);
    equal(w.getBaseForm("terrible","z"), null);

  }*/ // Not in API at present

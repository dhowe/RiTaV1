...
/*    
    ...
    for (int i = 0; i < words.length; i++)
    {
      ...
      if (phones == null || phones.length()<1)
      {
        ...
        if (ltsPhones != null && ltsPhones.length > 0) {
          ...
          phones = words[i];  // still nothing, use the raw word (should be punct)
          useRaw = true;
        }
      }
  ...
  public RiString insertWord(int wordIdx, String newWord)
  {
    ...
    wordIdx = Math.min(wordIdx < 0  ? (words.length + wordIdx) % words.length: wordIdx, words.length); 
  ...
  public String getPosAt(int wordIdx, boolean useWordNetTags) {
    ...
    wordIdx = Math.min(wordIdx < 0  ? (pos.length + wordIdx) % pos.length : wordIdx, pos.length); // -index?
  ...
  public String posAt(int wordIdx, boolean useWordNetTags) {
    ...
    // TODO: a bug - length of words() is diff (longer, if punctuation) than length of pos()
  ...
  public RiString replaceWordAt(int wordIdx, String newWord)
  {
    ...
    String[] words = words();
    wordIdx = Math.min(wordIdx < 0  ? words.length + wordIdx : wordIdx, words.length-1);
  ...
  /**
   * Tokenizes the RiString into words, then checks for features with the same number of elements 
   * as the resulting array and adds the appropriate feature to each newly created RiString<br>
   * Example: <br>
...
/*    if (end < start){
  ...
  public String wordAt(int wordIdx) {
    ...
    String[] words = RiTa.tokenize(delegate.toString());
    if (words == null || words.length < 1) return E;
   ...
   *   (with optional stress numbers after vowels), e.g.  ["B", "AE1", "T"] or ["B", "AE", "T"]
   *       
   *  @exclude private
   *  
  ...
  public static String syllabify(String[] sylls) {
    ...
    List<String[][]> syllables = new ArrayList<String[][]>(); // returned data structure: 3D array
    ...
    for (int i = 0; i < sylls.length; i++) {
      ...
      if (dbug)System.out.println
         (i+")"+phoneme + " stress="+stress+" inter="+RiTa.join(internuclei,":"));
      ...
      if (inArray((String[]) Phones.get("vowels"), phoneme)) {
        ...
        for (int split = 0; split < internuclei.size()+1; split++) {
          ...
          if (dbug)System.out.println("  "+split+") onset="+RiTa.join(onset,":")+
            "  coda="+RiTa.join(coda,":")+"  inter="+RiTa.join(internuclei,":"));
          
          // If we are looking at a valid onset, or if we're at the start of the word
          // (in which case an invalid onset is better than a coda that doesn't follow
          // a nucleus), or if we've gone through all of the onsets and we didn't find
          // any that are valid, then split the nonvowels we've seen at this location.
          boolean bool = inArray((String[]) Phones.get("onsets"), RiTa.join(onset," "));
        ...
        if (syllables.size() > 0 ) {
          ...
          //System.out.println('  len='+syllables[syllables.length-1][3].length);
          
          //extend(syllables[syllables.length-1][3], coda);
          String[][] last = syllables.get(syllables.size()-1);
          last[3] = extend(last[3], coda);
          
          //if (dbug) System.out.println("  tack: "+coda+" -> len="+syllables[syllables.length-1][3].length+" ["+syllables[syllables.length-1][3]+"]");
        }
      ...
      else if (!inArray((String[]) Phones.get("consonants"), phoneme) && !phoneme.equals(SP)) {
  ...
  static
  {
    Phones.put("consonants", new String[] { "b", "ch", "d", "dh", "f", "g", "hh", "jh",
        "k", "l", "m", "n", "ng", "p", "r", "s", "sh", "t", "th", "v", "w", "y", "z",
        "zh" });

    Phones.put("vowels", new String[] { "aa", "ae", "ah", "ao", "aw", "ax", "ay", "eh",
        "er", "ey", "ih", "iy", "ow", "oy", "uh", "uw" });

    Phones.put("onsets", new String[] { "p", "t", "k", "b", "d", "g", "f", "v", "th",
        "dh", "s", "z", "sh", "ch", "jh", "m", "n", "r", "l", "hh", "w", "y", "p r",
        "t r", "k r", "b r", "d r", "g r", "f r", "th r", "sh r", "p l", "k l", "b l",
        "g l", "f l", "s l", "t w", "k w", "d w", "s w", "s p", "s t", "s k", "s f",
        "s m", "s n", "g w", "sh w", "s p r", "s p l", "s t r", "s k r", "s k w",
        "s k l", "th w", "zh", "p y", "k y", "b y", "f y", "hh y", "v y", "th y", "m y",
        "s p y", "s k y", "g y", "hh w", "" });

    Phones.put("digits", new String[] { "z-ih-r-ow", "w-ah-n", "t-uw", "th-r-iy",
        "f-ao-r", "f-ay-v", "s-ih-k-s", "s-eh1-v-ax-n", "ey-t", "n-ih-n" });
  }
